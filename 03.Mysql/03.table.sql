# default  값으로 만들어진 데이터 테이블
CREATE TABLE datetable (
	id INT PRIMARY KEY AUTO_INCREMENT,
	dt DATETIME DEFAULT CURRENT_TIMESTAMP
);

DESC datetable;

INSERT INTO dateTable VALUES (DEFAULT, DEFAULT);
SELECT * FROM dateTable;									# => 입력하는 순간의 시간이 찍힘

SELECT DATE_FORMAT(dt, '%Y-%m-%d') FROM dateTable 			# 2022-09-23 라고 찍힘

SELECT DATE_FORMAT(dt, '%r') FROM dateTable					# 02:28:36 PM 라고 찍힘

SELECT DATE_FORMAT(dt, '%h:%i:%s %p') FROM dateTable		# 02:28:36 PM 라고 찍힘

SELECT NOW(), CURDATE(), CURTIME();							# 2022-09-23 14:35:11, 2022-09-23, 14:35:11


# 날짜 더하기/뺴기
SELECT DATE_ADD(NOW() , INTERVAL 40 DAY);					# 2022-11-02 14:36:28 라고 찍힘

# 날짜 더하기/뺴기
SELECT DATE_SUB(NOW() , INTERVAL 3 MONTH);					# 2022-06-23 14:37:27 라고 찍힘


# D-DAY 구하기 (수능)

SELECT TO_DAYS('2022-11-17') - TO_DAYS(NOW());				# 55

# 요일 구하기
SELECT DAYOFWEEK(dt) FROM dateTable;

# 요일 구하기 (h)
SELECT DATE_FORMAT(dt, '%y-%m-%d %h:%i') from dateTable;	# 22-09-23 02:28

# 요일 구하기 (H)
SELECT DATE_FORMAT(dt, '%y-%m-%d %H:%i') from dateTable;	# 22-09-23 14:28

#그룹생성
CREATE TABLE girl_group ( 
	gid INT PRIMARY KEY AUTO_INCREMENT, 
	name VARCHAR(32) NOT NULL, 
	debut DATE NOT NULL, 
	hit_song_id INT 
) AUTO_INCREMENT=1001;

CREATE TABLE song (
    sid INT PRIMARY KEY AUTO_INCREMENT,
    title VARCHAR(32) NOT NULL,
    lyrics VARCHAR(32)
) AUTO_INCREMENT=101;

# 걸그룹, 년도 넣기
INSERT INTO girl_group (name, debut, hit_song_id)
	VALUES ('원더걸스', '2007-02-10', 101),
	('소녀시대', '2007-08-02', 102), ('카라', '2009-07-30', 103),
	('브라운아이드걸스', '2008-01-17', 104), ('다비치', '2009-02-27', 105),
	('2NE1', '2009-07-08', 106), ('f(x)', '2011-04-20', 108),
	('시크릿', '2011-01-06', 109), ('레인보우', '2010-08-12', 110),
	('애프터 스쿨', '2009-11-25', 120), ('포미닛', '2009-08-28', 121);

SELECT * FROM girl_group; 											# 걸그룹 확인
SELECT * FROM song;													# 노래 확인
SELECT COUNT(*) FROM song;											# 갯수
SELECT COUNT(*) FROM girl_group;									# 갯수가

# inner join
SELECT gg.gid, gg.name, s.title
FROM girl_group AS gg
INNER JOIN song AS s # JOIN song AS s
ON s.sid = gg.hit_song_id;

#  Left Outer, Right Outer Join
SELECT gg.gid, gg.name, s.title
FROM girl_group AS gg
LEFT OUTER JOIN song AS s # LEFT JOIN song AS s
ON s.sid = gg.hit_song_id;

SELECT s.sid, s.title, gg.name
FROM girl_group AS gg
RIGHT OUTER JOIN song AS s # RIGHT JOIN song AS s
ON s.sid = gg.hit_song_id;

# 걸그룹이름, 데뷔, 타이틀, 가사 나오게 하기
SELECT gg.name, gg.debut, s.title, s.lyrics
	FROM girl_group AS gg
	INNER JOIN song AS s
	ON s.sid = gg.hit_song_id;

# 데뷔 일자가 빠른 5개 걸그룹의 히트송? (그룹명, 곡명)

SELECT gg.name, gg.debut, s.title
	FROM song AS s
	JOIN girl_group AS gg
	ON s.sid = gg.hit_song_id
	ORDER BY gg.debut
	LIMIT 5;

# 2007년도에 데뷔한 걸그룹은?
SELECT NAME, debut FROM girl_group
	WHERE debut BETWEEN DATE('2007-01-01') AND DATE ('2007-12-31');

# 2009년도에 데뷔한 걸그룹의 히트송 (걸그룹이름, 데뷔일, 히트송)
SELECT gg.name, gg.debut, s.title
	FROM girl_group AS gg
	JOIN song AS s
	WHERE gg.debut BETWEEN DATE('2009-01-01') AND DATE('2009-12-31');


# 히트곡 넣기
INSERT INTO song (title, lyrics)
	VALUES ('Tell Me', 'tell me tell me tetetete tel me'),
	('Gee', 'GEE GEE GEE GEE GEE BABY BABY'),
	('미스터', '이름이 뭐야 미스터'),
	('Abracadabra', '이러다 미쳐 내가 여리여리'),
	('8282', 'Give me a call Baby baby'), ('기대해', '기대해'),
	('I Don\'t care', '다른 여자들의 다리를'),
	('Bad Girl Good Girl', '앞에선 한 마디 말도'), ('피노키오', '뉴예삐오'),
	('별빛달빛', '너는 내 별빛 내 마음의 별빛'),
	('A', 'A 워오우 워오우워 우우우'),
	('나혼자', '나 혼자 밥을 먹고 나 혼자 영화 보고'), ('LUV', '설레이나요 '),
	('짧은치마', '짧은 치마를 입고 내가 길을 걸으면'),
	('위아래', '위 아래 위위 아래'), ('Dumb Dumb', '너 땜에 하루종일');
